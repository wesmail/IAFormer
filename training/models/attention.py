...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_dim, u_embed=6, num_heads=1, dropout=0.0):
        ...
        self.q_proj = nn.LazyLinear(embed_dim, bias=False)
        self.k_proj = nn.LazyLinear(embed_dim, bias=False)
        self.v_proj = nn.LazyLinear(embed_dim, bias=False)
        self.out_proj = nn.LazyLinear(
            embed_dim
        )  # Linear output layer to combine head outputs
...
class MultiHeadDiffAttention(nn.Module):
    ...
    def forward(self, x, u, umask=None):
        ...
        lambda_full = (lambda_1 * self.lambda_init).clamp_(max=1)
...
class ParticleAttentionBlock(nn.Module):
    ...
    def forward(self, x, u=None, umask=None):
        x_res = x
        x = self.norm1(x)
        attn_output, attn_weights = self.pmha(x, u, umask)  # x, and u embeddings
