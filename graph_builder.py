...
class ParticleGraphBuilder:
    ...
    def process_chunk(self, start, end):
        ...
        sparse_adj_matrix = np.stack(
            [
                ak.to_numpy(ak.fill_none(ak.pad_none(delta, target=pad_max), -1000)),
                ak.to_numpy(ak.fill_none(ak.pad_none(k_T, target=pad_max), -1000)),
                ak.to_numpy(ak.fill_none(ak.pad_none(z, target=pad_max), -1000)),
                ak.to_numpy(
                    ak.fill_none(ak.pad_none(m_squared, target=pad_max), -1000)
                ),
            ],
            axis=-1,
        )
...
class MemMapParticleGraphBuilder:
    def __init__(
        self,
        file_path: str,
        key: str = "table",
    ...
    def generate_graphs(self, output_file="output", n_jobs=1):
        ...
        batch_size = n_jobs
        # Initialize memmap files for writing (will be set on the first batch)
        lorentz_memmap = np.memmap(
            f"{output_file}_feature_matrix.npy",
            dtype="float32",
            mode="w+",
            shape=(total_rows, self.max_particles, 11),
        )
        adj_memmap = np.memmap(
            f"{output_file}_adj_matrix.npy",
            dtype="float32",
            mode="w+",
            shape=(total_rows, self.max_particles * (self.max_particles - 1) // 2, 4),
        )
        mask_memmap = np.memmap(
            f"{output_file}_mask.npy",
            dtype="int16",
            mode="w+",
            shape=(total_rows, self.max_particles),
        )
        label_memmap = np.memmap(
            f"{output_file}_labels.npy",
            dtype="int16",
            mode="w+",
            shape=(total_rows,),
        )
        ...
        for i in range(0, len(chunk_ranges), batch_size):
            ...
            for j, (start, end) in enumerate(batch_ranges):
                ...
                lorentz_memmap[start:end] = features
                adj_memmap[start:end] = adj_matrices
                mask_memmap[start:end] = mask
                label_memmap[start:end] = labels
        ...
        # Flush memmap arrays to disk (good practice, though they usually flush on close)
        lorentz_memmap.flush()
        adj_memmap.flush()
        mask_memmap.flush()
        label_memmap.flush()

        logging.info(
            "Data successfully saved to %s_feature_matrix.npy, %s_adj_matrix.npy, etc.",
            output_file,
            output_file,
        )


class H5ParticleGraphBuilder:
    def __init__(
        self,
        file_path: str,
        key: str = "table",
